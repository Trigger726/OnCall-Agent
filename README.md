...
- MySQL 8.4 Testcontainers：Flyway V1-V15、中文数据、幂等复合唯一索引、Runbook BM25、完整 9 步/18 事件调查、复盘发布、逾期扫描/行动项完成，以及 Problem 创建、状态闭环；新增的 REPEATABLE READ 双事务并发提升正在进行第二轮远端复验。

默认后端套件发现 42 项测试：40 项执行通过，2 项 Docker-MySQL 条件测试默认跳过；覆盖合法长标题登记、原始证据保留和 H2 并发提升。另行启用条件测试后，MySQL 8.4 从空库执行 Flyway V1–V15，并验证到期快照清理与重复执行幂等、中文数据、Runbook 检索、完整调查链路、复盘发布、逾期扫描幂等、行动项关闭和 Problem 生命周期。新增的两个 REPEATABLE READ 事务旧快照并发用例在首次远端运行中暴露了冲突恢复仍沿用旧快照的问题，现已改为新事务完整恢复并等待第二轮远端确认。Flyway 9.22.3 会提示其官方测试上限为 MySQL 8.0，后续应升级依赖并继续保留真实数据库门禁。GitHub Actions 将前端构建、H2 后端测试与 JAR、MySQL Testcontainers、容器构建与健康启动拆成四个门禁。阶段性运行与界面证据见 [docs/acceptance/README.md](docs/acceptance/README.md)。
