...
public class ChatController {
    ...
    /**
     * 直接搜索内部文档（绕过Agent，直接调用RAG检索）
     * 用于前端快速搜索知识库
     */
    @GetMapping("/search-docs")
    public ResponseEntity<ApiResponse<Map<String, Object>>> searchDocs(@RequestParam("q") String query) {
        try {
            logger.info("收到直接搜索文档请求: {}", query);

            if (query == null || query.trim().isEmpty()) {
                return ResponseEntity.ok(ApiResponse.error("搜索关键词不能为空"));
            }

            // 直接调用 InternalDocsTools 进行检索（绕过 ReactAgent）
            String resultJson = internalDocsTools.queryInternalDocs(query.trim());

            Map<String, Object> resultMap = new HashMap<>();
            resultMap.put("query", query);
            resultMap.put("rawResult", resultJson);

            // 尝试解析结果判断是否成功
            try {
                // rawResult is already a JSON string from InternalDocsTools
                if (resultJson.contains("\"status\":\"error\"") || resultJson.contains("\"status\": \"error\"")) {
                    resultMap.put("success", false);
                    // Extract the error message
                    String hint = "知识库搜索失败。";
                    if (resultJson.contains("403") || resultJson.contains("AccessDenied") || resultJson.contains("Access denied")) {
                        hint = "DashScope Rerank API 权限不足 (403 Forbidden)。请在阿里云百炼控制台开通 gte-rerank 模型服务，或在 application.yml 中设置 rerank.enabled: false 跳过重排序。";
                    } else if (resultJson.contains("Connection refused") || resultJson.contains("Failed to connect")) {
                        hint = "Milvus 向量数据库未启动。请确保 Milvus 在 localhost:19530 运行。";
                    }
                    resultMap.put("hint", hint);
                } else if (resultJson.contains("\"status\":\"no_results\"") || resultJson.contains("\"status\": \"no_results\"")) {
                    resultMap.put("success", true);
                    resultMap.put("hint", "知识库中未找到相关文档。请尝试其他关键词或先上传文档到知识库。");
                } else {
                    resultMap.put("success", true);
                }
            } catch (Exception parseEx) {
                resultMap.put("success", false);
                resultMap.put("hint", "结果解析失败: " + parseEx.getMessage());
            }
            ...
            errorMap.put("hint", "搜索失败，可能原因：Milvus 服务未启动、知识库为空、或网络异常。请检查 Milvus 是否在 localhost:19530 运行。");
            return ResponseEntity.ok(ApiResponse.success(errorMap));
        }
    }
    ...
    public ResponseEntity<Map<String, Object>> health() {
        ...
        health.put("service", "SuperBizAgent");
     ...
     * 导出会话对话记录（Markdown格式）
     */
    @GetMapping("/chat/export/{sessionId}")
    public ResponseEntity<ApiResponse<Map<String, String>>> exportConversation(@PathVariable String sessionId) {
        try {
            logger.info("收到导出会话请求 - SessionId: {}", sessionId);
            ...
            markdown.append("# SuperBizAgent 对话记录\n\n");
            markdown.append("> 导出时间: ").append(new java.util.Date()).append("\n");
    ...
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> listSessions() {
        try {
            ...
            for (Map.Entry<String, SessionInfo> entry : sessions.entrySet()) {
                ...
                info.put("messagePairCount", entry.getValue().getMessagePairCount());
                info.put("createTime", entry.getValue().createTime);
