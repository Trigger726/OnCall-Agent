...
| 阶段 | 原 OnCall/V1 | V1.6 | V1.7 checkpoint 01 |
| --- | --- | --- | --- |
| 事故恢复后 | 对话结束，无正式复盘 | 可检索/评测 Runbook | 从真实证据生成无责复盘快照 |
| 防复发治理 | 自然语言建议 | 相关性反馈形成 qrel | 负责人 + 截止日期 + 完成状态 |
| 质量门禁 | 无 | 检索判断独立复核 | 复盘提交人与发布人分离 |
| 并发与追溯 | 无 | 判断版本与审计 | 父子乐观锁、时间线、审计、发布冻结 |

历史原型、V1 内置 Runbook 和 V1.6 加演均保留。这里不演示尚未实现的逾期通知、跨事故趋势或 MTTR 看板。
