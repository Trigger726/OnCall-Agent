...
checkpoint 05 工程加演：展示真实 MySQL 双事务测试。旧方案在唯一键冲突后执行普通回读，REPEATABLE READ 旧快照存在看不到赢家记录的风险；新版改用加锁当前读。测试先让两个事务都确认候选未登记，再同时放行，最终一创一复用、同一 Problem 主键、两条 Incident 关联和两条时间线。这个加演用于讲数据库隔离级别与幂等设计，不替代上面的产品页面演示。
