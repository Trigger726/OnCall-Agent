...
强调这不是生产准确率：13 条是种子集，演示中的第 14 条和 κ 都来自隔离环境样本；测试替身只验证索引、融合和失败契约，不能证明真实 Embedding 质量。真实历史样本、第三方仲裁、cross-encoder rerank、ANN、OCR 和内容审核仍是后续项。历史原型分支及 V1 内置 Runbook 均保留，可独立演示。
