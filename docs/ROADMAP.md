...
- 检查点 05 本地完成：MySQL 条件套件新增 REPEATABLE READ 双事务确定性闸门，两个事务先建立“记录不存在”快照再同时提升；冲突回收改用 `SELECT ... FOR UPDATE` 当前读，断言一创一复用、同一主键及关联/时间线不重复。默认 42 项中 40 项通过、2 项 MySQL 条件测试跳过，最终远端门禁待验。
