...
- 待完成：有效排班的维护入口、第三方值班渠道与人工已读/接单回执；Runbook 命中率趋势、跨 Incident 语义相似/依赖共因聚类；生产业务 exporter/服务指标与 Prometheus 规则联调、管理端口生产网络/鉴权边界，以及 SLO 出站通知、生产 recording rules/长期数据和低流量样本策略；Trace 的真实生产 Prometheus/Loki 联调、Collector 重启持久化与生产容量验证。
