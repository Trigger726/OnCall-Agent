...
- 检查点 42 本地通过：P1 未确认事故按现有 ON_CALL/USER/ROLE 策略执行，V24 唯一台账和行锁防重，缺班记 `NO_TARGET`、确认后停止；保留旧班次/策略 Demo 并增加桌面/移动执行记录。H2 125 项发现/110 项执行、前端 13 项及真实 JAR/API/页面通过，MySQL 与远端容器待验。详见 [验收记录](acceptance/V1.7-checkpoint-42.md)。
