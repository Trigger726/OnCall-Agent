...
- 检查点 05 修复复验中：MySQL 条件套件新增 REPEATABLE READ 双事务确定性闸门，两个事务先建立“记录不存在”快照再同时提升。首次远端运行证明只把主键回读改成 `SELECT ... FOR UPDATE` 仍不足，后续普通读取继续命中旧快照；冲突恢复现已整体移入 `REQUIRES_NEW` 新事务。默认 42 项中 40 项通过、2 项 MySQL 条件测试跳过，第二轮远端门禁待验。
