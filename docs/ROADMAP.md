...
- 检查点 25 进行中：新增仅用于验收的独立 instrumented Provider fixture；Trace 容器门禁已接入 Prometheus/Loki 兼容 HTTP 查询，准备从 Tempo 原始 trace 验证两个 `OpsPilot CLIENT -> fixture SERVER` 的共享 traceId 与严格父子 ID，并在 Tempo 短暂停机恢复场景重复验证。本机 fixture JAR 和协议响应已通过；真实容器/Tempo 结果待远端 CI，不提前宣称跨服务验收完成。
...
- 待完成：Runbook 命中率趋势、跨 Incident 语义相似/依赖共因聚类、服务/SLO 目标线、真实外部提醒与回执；Trace 的真实生产 Prometheus/Loki 联调、Collector 重启持久化与生产容量验证。checkpoint-25 的跨 JVM fixture/Tempo 门禁仍待远端结果。
