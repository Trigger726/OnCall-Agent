...
- 检查点 36 已完成：独立 Node 接收进程与隔离 Compose 门禁覆盖真实后台调度、503→204 重试、稳定幂等键和重复扫描零新增；[Run 35859792690](https://github.com/Trigger726/OnCall-Agent/actions/runs/35859792690) 十一项 CI 全绿，含容器联调与工件。真实第三方平台、人工确认和页面浏览器仍待验。详见 [验收记录](acceptance/V1.7-checkpoint-36.md)。
- 检查点 37 通过（限本检查点）：负责人登录确认接手独立于 webhook 回执与完成状态，V23 持久化确认人/时间，角色/本人限制、并发幂等、时间线、审计与双页面入口已实现。[Run 35861441244](https://github.com/Trigger726/OnCall-Agent/actions/runs/35861441244) 十一项 CI 全绿，含 MySQL 8.4 双线程确认；本地 JAR 的 HTTP/重启回归也通过。2026-09-25 补验桌面及 390px 浏览器双入口点击、刷新持久化和独立完成状态；真实第三方渠道、跨浏览器兼容性仍待验。详见 [验收记录](acceptance/V1.7-checkpoint-37.md)。
...
- 检查点 41 通过（限本检查点）：Actuator 从业务监听剥离，JAR 默认回环管理端口、Compose 宿主机回环映射和内部 Prometheus 抓取配置已更新；本地 H2 119 项发现/105 项执行、前端 13 项及 JAR 真实双端口通过，[Run 36150079204](https://github.com/Trigger726/OnCall-Agent/actions/runs/36150079204) 的 MySQL 8.4、服务指标容器联调与十一项 CI 全绿。详见 [验收记录](acceptance/V1.7-checkpoint-41.md)。
- 检查点 42 通过（限本检查点）：P1 未确认事故按现有 ON_CALL/USER/ROLE 策略执行，V24 唯一台账和行锁防重，缺班记 `NO_TARGET`、确认后停止；保留旧班次/策略 Demo 并增加桌面/移动执行记录。UTC 时钟混用已复现并统一数据库时钟修复，本地双时区各 125 项发现/110 项执行、0 失败及最新 JAR UTC HTTP 通过；[Run 36260824938](https://github.com/Trigger726/OnCall-Agent/actions/runs/36260824938) 的 MySQL 与十一项 CI 全绿，桌面/390px 截图及 CI/HTTP 证据已归档。详见 [验收记录](acceptance/V1.7-checkpoint-42.md)。
- 检查点 43 已通过：新增普通/临时覆盖班次维护、计划行锁防同层重叠、半开交接区间、版本化软取消与审计；取消覆盖恢复普通班次，历史记录不变。双时区各 132 项发现/116 项执行、前端 13 项、桌面/390px 与最新 JAR 四个新事故的真实路由已验；[Run 36262966874](https://github.com/Trigger726/OnCall-Agent/actions/runs/36262966874) 的 V25/真实 MySQL 并发与十一项远端门禁全部成功。详见 [验收记录](acceptance/V1.7-checkpoint-43.md)。
- 检查点 44 已通过：已复现接入/扫描旧快照忽略已提交取消的误路由，事务入口改为显式 READ_COMMITTED；H2 八项提交闸门/原子回滚场景与双时区全量通过（148 项发现/124 项执行/24 项 Docker 跳过），前端 13 项通过。[Run 36264417669](https://github.com/Trigger726/OnCall-Agent/actions/runs/36264417669) 十一项 CI 全绿，直接日志确认真实 MySQL 新套件 8 项加原套件 9 项全部执行、零跳过。详见 [验收记录](acceptance/V1.7-checkpoint-44.md)。
- 待完成：班次自动轮转、跨时区/DST、换班请求与日历同步，第三方值班渠道与人工已读/接单回执；Runbook 命中率趋势、跨 Incident 语义相似/依赖共因聚类；生产业务 exporter/服务指标与 Prometheus 规则联调、管理端口生产网络/鉴权边界，以及 SLO 出站通知、生产 recording rules/长期数据和低流量样本策略；Trace 的真实生产 Prometheus/Loki 联调、Collector 重启持久化与生产容量验证。
