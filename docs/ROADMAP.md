...
- 检查点 42 续验中：P1 未确认事故按现有 ON_CALL/USER/ROLE 策略执行，V24 唯一台账和行锁防重，缺班记 `NO_TARGET`、确认后停止；保留旧班次/策略 Demo 并增加桌面/移动执行记录。上一轮前端及真实 JAR/API/页面通过，远端 MySQL 通过；UTC H2 失败已复现并统一数据库时钟修复，本地双时区各 125 项发现/110 项执行、0 失败及 JAR 打包通过，修复后的远端门禁待补证。详见 [验收记录](acceptance/V1.7-checkpoint-42.md)。
