...
- 检查点 44 本地通过：已复现接入/扫描旧快照忽略已提交取消的误路由，事务入口改为显式 READ_COMMITTED；H2 八项提交闸门/原子回滚场景与双时区全量通过（148 项发现/124 项执行/24 项 Docker 跳过），前端 13 项通过。真实 MySQL 同场景已加入 CI，待远端补证。详见 [验收记录](acceptance/V1.7-checkpoint-44.md)。
