...
- 检查点 13：新增 GET SSE、连接/发送任务上限、终态关闭与数据库定期补读，正在验证；前端自动恢复与双 Web 实例演示仍待完成。
...
- 跨实例实现决策见 [ADR-001](ADR-001-distributed-agent-events.md)：同事务 outbox + 独立 XREAD 广播 + 数据库补读 + 前端游标恢复。当前前端没有自动回放，完整分布式链路仍未验收；下一步接入本地订阅与数据库补读。
