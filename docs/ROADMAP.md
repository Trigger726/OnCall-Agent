...
- 检查点 43 本地通过：新增普通/临时覆盖班次维护、计划行锁防同层重叠、半开交接区间、版本化软取消与审计；取消覆盖恢复普通班次，历史记录不变。双时区各 132 项发现/116 项执行、前端 13 项、桌面/390px 与最新 JAR 四个新事故的真实路由已验；V25/真实 MySQL 与远端门禁待补证。详见 [验收记录](acceptance/V1.7-checkpoint-43.md)。
