...
- 检查点 14：前端 POST → GET 自动续订、游标去重和退出中止，9 项前端故障测试与 Incident 新旧桌面/移动端对照通过；远端 CI、OnCall 独立实页、双实例与刷新后自动挂接待验。
...
- 跨实例实现决策见 [ADR-001](ADR-001-distributed-agent-events.md)：同事务 outbox + 独立 XREAD 广播 + 数据库补读 + 前端游标恢复。各层已分阶段接入；完整双实例链路、运行中故障注入与刷新后自动挂接仍未验收。
